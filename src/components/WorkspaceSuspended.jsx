...
const WorkspaceSuspended = () => {
  ...
  const tenantName = user?.tenantName || user?.tenant?.name || user?.workspace || "Enterprise Workspace";
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-4">
            <span className="flex items-center gap-1.5 px-3 py-1 bg-rose-500/20 text-rose-400 border border-rose-500/30 rounded-full text-xs font-black uppercase tracking-wider">
              <ShieldAlert size={14} /> Workspace Access Suspended
            </span>
          ...
          <h1 className="text-2xl font-black text-white tracking-tight mb-3">
            Organization Access Temporarily Paused
          </h1>
          
          <p className="text-slate-400 text-xs leading-relaxed mb-5 font-medium">
            Access to your company workspace (<strong className="text-white">{tenantName}</strong>) has been temporarily suspended by system administration due to SLA status or subscription updates.
          </p>
          ...
          <div className="p-3.5 bg-slate-900/90 rounded-2xl border border-slate-800 space-y-1.5 text-xs font-medium text-slate-300">
            <div className="flex items-center gap-2 text-rose-400 font-bold">
              <HelpCircle size={14} /> Notice for Employees &amp; Managers:
            </div>
            <p className="text-slate-400 leading-relaxed text-[11px]">
              During workspace suspension, portal navigation, team skill matrix views, and API operations are temporarily disabled for all organization members.
            </p>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Contact SkillPulse Support via official email.</p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <a 
                  href={`mailto:${supportEmail}?subject=Workspace%20Reactivation%20Request%20-${encodeURIComponent(tenantName)}`}
            ...
            <div className="flex items-center gap-2">
              ...
              <a
                href={`mailto:${supportEmail}?subject=Workspace%20Reactivation%20Request%20-${encodeURIComponent(tenantName)}&body=Hello%20Support,%0A%0AOur%20organization%20workspace%20(${encodeURIComponent(tenantName)})%20has%20been%20suspended.%20Please%20help%20us%20resume%20our%20subscription%20access.%0A`}
          ...
          <div className="pt-2 flex justify-between items-center border-t border-slate-800/80">
            <span className="text-[11px] text-slate-500">Contact Super Admin or Support to clear account hold.</span>
